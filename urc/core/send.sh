...
# Compare bottom-of-pane before/after — if unchanged, retry Enter.
# Skip for relay control messages (low risk, adds latency to relay path).
case "$TEXT" in
  __urc_push__|__urc_refresh__|/remote-control) ;;
  *)
    case "$CLI" in
...
case "$TEXT" in
  __urc_push__|__urc_refresh__|/remote-control|"You have an unread message from %"*) ;;  # nudge format: server.py send_message()
  *)
    ...
    if [ -n "$_relay" ]; then
      ...
      jq -n --arg pane "$PANE" --arg cli "$CLI" --arg status "processing" \
        --arg message "${TEXT:0:100}" --argjson epoch "$(date +%s)" \
        '{pane:$pane,cli:$cli,epoch:$epoch,status:$status,message:$message}' \
        > "$_push_dir/${_relay}_${PANE}_processing_$(date +%s).json" 2>/dev/null
      (bash "${SCRIPT_DIR}/send.sh" "$_relay" "__urc_push__" --cli claude >/dev/null 2>&1) &
    fi
    ;;
esac
